...
function rangedWeaponRanges(npcId, ranged)
  ...
  local ranges = gunRanges[item] or gunRanges.default or {}
  return {
    minRange = 30,
    maxRange = 120,
    forceMoveRange = 80
...
function meleeWeaponRanges(npcId, ranged)
  local item = world.entityHandItem(npcId, "primary")
  local weaponRanges = config.getParameter("npcCombat.meleeWeaponRanges") or {}
  local ranges = weaponRanges[item] or weaponRanges.default or {}
  return {
    minRange = ranges.minRange or 5,
    maxRange = ranges.maxRange or 10
  }
end
...
function setRangedAttackerPositions()
  ...
  if self.tasks["ranged"] then
    ...
    local memberRanges = util.map(self.tasks["ranged"].members, function(memberId)
      ...
      self.memberResources[memberId]:set("maxRange", ranges.maxRange)
      self.memberResources[memberId]:set("forceMoveRange", ranges.forceMoveRange)
      self.memberResources[memberId]:set("minRange", ranges.minRange - 1) --
      return {memberId, ranges}
    end)
