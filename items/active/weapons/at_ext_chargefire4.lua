...
function GunFire:charge()
  if self.isCharged == false then
    self.weapon:updateAim()
    self.weapon:setStance(self.stances.charge)
    animator.setAnimationState("firing", "charge")

      if self.stances.charge.duration then         -- charging
        util.wait(self.stances.charge.duration)
        self.isCharged = true
      animator.setAnimationState("firing", "charged")
      end
  else
...
function GunFire:auto()
  while self.fireMode == (self.activatingFireMode or self.abilitySlot) do  
    if (not world.lineTileCollision(mcontroller.position(), self:firePosition())) and status.overConsumeResource("energy", self:energyPerShot()) then
     ...
     if self.stances.fire2.duration then
       util.wait(self.stances.fire2.duration)
     end
     self.cooldownTimer = self.fireTime
