...
function init()
  ...
  self.vehicleState = config.getParameter("filled") and VehicleBlocked or VehicleEmpty
  self.respawnTimer = 0
  updateIcon()
  ...
  if config.getParameter("filled") then
    if fireMode == "primary" then
      if self.vehicleState == VehiclePlaceable then
        ...
        world.spawnVehicle(config.getParameter("vehicleType"), activeItem.ownerAimPosition(), {
...
function update(dt)
  if config.getParameter("filled") then
    if self.respawnTimer > 0 then
      ...
      self.vehicleState = VehicleEmpty
    elseif placementValid() then
      self.vehicleState = VehiclePlaceable
    else
      self.vehicleState = VehicleBlocked
    end
  elseif self.consumePromise and self.consumePromise:finished() then
...
function placementValid()
  ...
  if world.lineTileCollision(mcontroller.position(), aimPosition) then return false end
  local vehicleBounds = {
    self.vehicleBoundingBox[1] + aimPosition[1], self.vehicleBoundingBox[2] + aimPosition[2],
    self.vehicleBoundingBox[3] + aimPosition[1], self.vehicleBoundingBox[4] + aimPosition[2]
  }
  return not world.rectTileCollision(vehicleBounds, {"Null", "Block", "Dynamic", "Slippery"})
end
