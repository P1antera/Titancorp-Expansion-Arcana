...
function init()
  ...
  local refreshInterval = 10800 -- 3小时 
  ...
  if currentTime - self.lastRefreshTime >= refreshInterval or #self.storedRecipes == 0 then
    ...
    self.storedRecipes = generateRecipes(costPool, inventoryPool, specialPool, currentTime)
...
function generateRecipes(costPool, inventoryPool, specialPool, currentTime)
  local recipes = {}
  

  -- 计算当前时间所属的3小时时间段
  local secondsToday = (currentTime % 86400) -- 今天的秒数
  local periodIndex = math.floor(secondsToday / 10800) -- 3小时时间段索引
  

  local daysSinceEpoch = math.floor(currentTime / 86400)
  

  local shuffleSeed = daysSinceEpoch * 8 + periodIndex
      ...
      if #costPool > 0 then
        ...
        elseif isReagent then
          ...
          for j = 1, 3 do
            ...
            selectedItems[costItem] = true
            costCount = math.random(12, 22) 
            table.insert(costItems, {item = costItem, count = costCount})
          end
        else

          local costItem = costPool[math.random(1, #costPool)]
          costCount = math.random(18, 26) 
          table.insert(costItems, {item = costItem, count = costCount})
        end
