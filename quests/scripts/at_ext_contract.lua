...
require("/quests/scripts/at_ext_delivery.lua")
...
-- For Package delivery contract
function init()
  self.conditions = buildConditions()
  self.failConditions = buildFailConditions()
  self.packageItem = config.getParameter("package")
...
function buildConditions()
  ...
  for _,config in pairs(conditionConfig) do
	...
	elseif config.type == "atDelivery" then
      newCondition = buildATEXT1(config)
...
function questStart()
      if player.hasItem(self.packageItem) == false
	  then
	  player.giveItem(self.packageItem, 1)
	  end
...
function update(dt)
  ...
  if player.hasItem(self.packageItem) == false then
  quest.fail()
  end
