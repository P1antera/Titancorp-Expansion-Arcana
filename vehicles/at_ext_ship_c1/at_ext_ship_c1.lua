...
function initShip()
 ...
 self.started = false
 self.bombdir = 0
  storage.ammo = storage.ammo or 50
...
-- self healing
  if storage.health <= 2000 then
   
    self.healcool = self.healcool - 1
    if  self.healcool == 0 then
    storage.health = storage.health + 20
...
--启动/关闭
  if driver then
    if self.lastDriver == nil then
      ...
      animator.setAnimationState("ship", "up")
       self.started = true
...
if self.started == true and mcontroller.yVelocity() == 0 then
  animator.setAnimationState("bottomthrust", "idle")
   ...
   self.bombdir = mcontroller.xVelocity()
   if self.bombdir > 0 then
     self.bombdir = 1
     else if self.bombdir < 0 then
       self.bombdir = -1
       else
        self.bombdir = 0
      end
...
--           world.spawnProjectile("at_ext_bomb_fire", vec2.add(mcontroller.position(), fireOffset), entity.id(), {self.bombdir, 0}, false)
