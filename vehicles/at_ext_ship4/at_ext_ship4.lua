...
function gunrotate()
  self.angle = mcontroller.yVelocity() / self.moveSpeed * 0.5
  local diff = world.distance(vehicle.aimPosition("seat"), vec2.add(mcontroller.position(),{0, 0}))
...
function firePods()
  while true do
    ...
    if self.firing then
      ...
      animator.setAnimationState("frontcannonlit", "open")
 
      util.wait(0.1)
      
      if storage.ammo > 0 and self.firing then
        animator.setAnimationState("frontcannon", "load")
        animator.setAnimationState("frontcannonlit", "load")
      ...
      while self.firing do
        if frontLoaded then
          ...
          animator.setAnimationState("frontcannonlit", "fire")
          util.wait(0.1)
          ...
          world.spawnProjectile("at_ext_ship4", vec2.add(mcontroller.position(), fireOffset), entity.id(), aimVector(aimAngle,0.02), false)
      ...
      animator.setAnimationState("frontcannonlit", "close")
