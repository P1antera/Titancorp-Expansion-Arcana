...
---@class FireConfig 设置默认值
local FireConfig = {
    projectileType = "dragonfirelarge",
    projectileParam = {},
    burstCount = 1,
    burstTime = 1,
    fireType = "single" --- single , burst
}
...
function init()
    ...
    self.behavior = behavior.behavior(config.getParameter("behavior"),
        sb.jsonMerge(config.getParameter("behaviorConfig", {}), skillBehaviorConfig()), _ENV)
    ...
    message.setHandler("shot",
        function(_, _, aimPosition)
            animator.playSound("fire")
            animator.setAnimationState("body", "fire")
            droneFire(aimPosition)
        end)

    self.isHover = "controlled"
    -------------------------------------- changeHover
    ...
    ---@type FireConfig
    self.fireConfig = config.getParameter("fireConfig")
    ...
    if deathBehavior then
        self.deathBehavior = behavior.behavior(deathBehavior, config.getParameter("behaviorConfig", {}), _ENV,
            self.behavior:blackboard())
    end
...
function droneFire(aimPosition)
    sb.logInfo("fire")
    local differenceVector = vec2.sub(aimPosition, mcontroller.position())

    local config = self.fireConfig

    

    local projectileType = config.projectileType or FireConfig.projectileType
    local fireType = FireType[config.fireType or FireConfig.fireType]
    local param = config.projectileParam or FireConfig.projectileParam
    local burstTime = config.burstTime or FireConfig.burstTime

    if fireType == FireType.single then
        world.spawnProjectile(projectileType, mcontroller.position(), entity.id(), differenceVector, nil, param)
    elseif fireType == FireType.burst then
